            ...
            <div className=" mt-2 ">
               ...
               eventsdata.map((evnt)=>(
                  <div className="flex flex-col gap-5"> 
                   ...
                   <div className="text-white text-sm font-poppins flex items-center gap-2"><BiTimeFive fontSize={17}/> {evnt.start_time} - {evnt.end_time}</div>
                  ...
                  <div className="text-white"> Live </div>
                    ...
                    <div className=" py-3">
                       <h1 className="text-[#6d9ad1] font-poppins">{eventsdata[0].students.length} Students invited</h1>
                       <div className="flex  max-h-[12vh] overflow-auto mb-1 -space-x-1 flex-wrap">
                     {
                      eventsdata.map((student)=>(
                        student.students.slice(0, openstudents?  eventsdata[0].students.length : 10).map((list)=>(
                      ...
                      <div className="relative cursor-pointer">
                        {
                          eventsdata[0].students.length>=10 &&  <a onClick={()=>setStudents(true)} className="flex items-center justify-center w-10 h-10 text-xs font-medium text-white bg-gray-700 border-2 border-white rounded-full hover:bg-gray-600 dark:border-gray-800" >{eventsdata[0].students.slice(openstudents?  eventsdata[0].students.length :10, eventsdata[0].students.length).length  }</a>
                        }
